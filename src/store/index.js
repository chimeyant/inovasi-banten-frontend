...
export default new Vuex.Store({
  ...
  actions: {
    ...
    getAppInfo: async function({ commit, dispatch, state }, payload) {
      try {
        let { data: info } = await this.state.http.get("api/v2/info");
    ...
    getUserInfo: async function({ commit, dispatch, state }, payload) {
      try {
        let { data: user } = await this.state.http.get("api/v2/user-info");
    ...
    getMenus: async function({ commit, dispatch, state }, payload) {
      try {
        let { data: menus } = await this.state.http.get("api/v2/menus");
    ...
    signIn: async function({ commit, state }, payload) {
      try {
        ...
        } = await state.http.post("/api/v2/auth/token", {
    ...
    postUpdate: async function({ state, commit, dispatch }, payload) {
      try {
        ...
        let {
          data: { success, message, data, code, errors },
        } =
          state.page.key == "id"

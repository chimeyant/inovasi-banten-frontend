    ...
    <v-row class="pa-1">
      ...
      <v-col :cols="device.desktop ? `6` : `12`">
        <v-card class="animated animate__bounceIn rounded-0">
          <v-card-title :class="`white--text ` + theme.color + ` darken-3`">Konfirgurasi Informasi Aplikasi</v-card-title>
          ...
          <v-card-text>
            <v-row>
              <v-col cols="12">
                <v-img
                  :class="[theme.color + ' darken-3 with-backdrop']"
                  :aspect-ratio="16 / 9"
                  :src="bg"
                >
                  <v-row
  ...
  methods: {
    ...
    fetchRecord: async function () {
      let { data } = await this.http.get(
        "api/v2/superadmin/master-data/app-info"
      );
    ...
    fetchUpdate: async function () {
      try {
        let {
          ...
          "api/v2/superadmin/master-data/app-info/" + this.record.id,
          this.record
        );
